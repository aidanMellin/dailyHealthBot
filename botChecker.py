...
async def todo(ctx, *todo_arg):
    ...
    if todo_arg[0] == "test":
        todo_add(str(ctx.message.author.id),["","test1"])
        todo_add(str(ctx.message.author.id),["","test2"])
        todo_add(str(ctx.message.author.id),["","test3"])
        await ctx.channel.send("TODO Test Completed")
        resp = todo_view(str(ctx.message.author.id))
        await ctx.channel.send(resp)

    elif todo_arg[0] == 'add': #Adding things to todo list
        todo_add(str(ctx.message.author.id),todo_arg)
        await ctx.channel.send("TODO added")

    elif 'remove' in todo_arg[0] or "rm" in todo_arg[0]: #Remove a (batch) of todo(s)
        todo_rm(str(ctx.message.author.id), todo_arg)
        ...
        todo_p(str(ctx.message.author.id),todo_arg)
        await ctx.channel.send("TODO Prioritized")

    elif todo_arg[0] == "clear":
        open(str(ctx.message.author.id)+".txt", 'w').close()
